...
import { Productd } from './';

export const Products = () => {
    const [productsData, setProductsData] = useState({data:'', status:0});

    const  dataResult = async () =>{
        const {data,status} = await getProducts();
        setProductsData({data, status});
    }
    ...
    <div className='flex flex-wrap justify-center' >
        {
            productsData.status === 200 ? 
                productsData.data.map( (product)=>{
                    return(
                        <Productd key={product.id} title={product.title} price={product.price} image={product.images} idProduct={product.id} />
                    )
                } ) 
            :
